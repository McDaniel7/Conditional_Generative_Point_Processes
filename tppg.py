...
class RayleighKernel(object):
    ...
    def nu(self, t, his_t):
        delta_t = t - his_t
        return delta_t / self.sigma**2 * np.exp(- delta_t**2 / 2 / self.sigma**2)
...
class SelfCorrectingLam(object):
    """Intensity of self-correcting point process in Du, et al. (2015)"""
    def __init__(self, mu, alpha, maximum=1e+4):
        self.mu      = mu
        self.alpha   = alpha
        self.maximum = maximum
    ...
    def value(self, t, his_t):
        ...
        occurred.
        """
        val = np.exp(self.mu * t - self.alpha * len(his_t))
        return val
...
def plot_1d_pointprocess_lam_f(points, lam, T, ngrid=100):
    """
    visualize 1 dimensional point process
    """
    ts      = np.linspace(T[0], T[1], ngrid)
    ...
    plot_points = points[(points <= T[1]) * (points >= T[0])]
    zros        = np.zeros((len(plot_points)))
    ...
    ax1.plot(ts, lamvals, linestyle="--", color="grey")
    ax1.scatter(plot_points, evals, c="r")

    ax2 = fig.add_subplot(122)
    ax2.plot(ts, fvals, linestyle="--", color="grey", label="$f(t)$")
    ax2.scatter(plot_points, zros - 0.1, c="r")
    plt.show()
