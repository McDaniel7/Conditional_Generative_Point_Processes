...
def plot_spatial_density(lam, points, S, t_slots, grid_size, interval):
    """
    Plot spatial intensity as the time goes by. The generated points can be also
    plotted on the same 2D space optionally.
    """
    ...
    x_span = np.linspace(S[1][0], S[1][1], grid_size+1)[:-1]
    y_span = np.linspace(S[2][0], S[2][1], grid_size+1)[:-1]
    loc_unit_vol = np.prod([S_k[1] - S_k[0] for S_k in S[1:]]) / (grid_size - 1)**2
    time_unit_vol = (S[0][1] - S[0][0]) / t_slots
    ...
    print('[%s] preparing the dataset %d × (%d, %d) for plotting.' %
        (arrow.now(), t_slots, grid_size, grid_size), file=sys.stderr)
    lamvals = [heatmap(t) for t in tqdm(t_span)]
    lamvals = np.array(lamvals)
    
    print('[%s] calculating fs...' % arrow.now())
    integral = []
    for i, t in tqdm(enumerate(t_span)):
        sub_seq_t = seq_t[seq_t < t]
        last_t = sub_seq_t[-1] if len(sub_seq_t) > 0 else 0
        last_idx = (t_span < last_t).sum()
        lam_loc_sum = lamvals[last_idx:(i+1)].sum((-1, -2)) * loc_unit_vol
        lam_sum = (np.array([t_span[last_idx] - last_t] + [time_unit_vol] * (len(lam_loc_sum) - 1)) * lam_loc_sum).sum()
        integral.append(lam_sum)
    integral = np.array(integral)
    fvals  = lamvals * np.exp(-integral)[:, None, None]

    # initiate the figure and plot
    fig = plt.figure()
    # set the image with largest total intensity as the intial plot for automatically setting color range.
    # im  = plt.imshow(data[data.sum(axis=-1).sum(axis=-1).argmax()], cmap='hot', animated=True) 
    im  = plt.imshow(fvals[-1], cmap='hot', animated=True) 
    # function for updating the image of each frame
    def animate(i):
        # print(t_span[i])
        im.set_data(fvals[i])
